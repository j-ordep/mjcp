...
A Home exibia "Minhas Escalas" com uma lista completa de eventos do usuário
...
Escalas (Calendar icon) - Minhas Escalas completas
Salas (DoorOpen icon) - Locação de salas
Músicas (Music icon) - Biblioteca de músicas
Perfil (User icon) - Perfil do usuário
Arquivos Criados/Modificados:
Criado:
/components/MySchedulesScreen.tsx - Tela dedicada para lista completa das escalas do usuário
Modificados:
...
Clicar em "Minhas Escalas" → MySchedulesScreen (também ativa a tab "Escalas")
Clicar em "Eventos da Igreja" → EventsScreen (modal/overlay)
Clicar em "Bloquear Datas" → BlockDatesScreen (modal/overlay)
Clicar em "Solicitar Troca" → Alert (funcionalidade futura)
Clicar em evento do resumo → EventDetailsScreen
Clicar em "Ver todas" (escalas) → MySchedulesScreen
Clicar em "Ver todos" (eventos) → EventsScreen
A partir da Tab Bar:
Tab "Escalas" → Vai direto para MySchedulesScreen
