...
test("replaceEventSetlist rewrites positions in selection order", async () => {
  createUpcomingEventsMock([]);
  ...
  assert.equal(result.error, null);
  assert.equal(calls.eventSetlistDeletes, 1);
  assert.deepEqual(calls.eventSetlistInserts[0][0], [
    {
      event_id: "event-1",
      song_id: "song-1",
      song_key: null,
